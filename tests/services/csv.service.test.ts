...
const service = new CSVService(mockLogger)

const INPUT_PATH = 'data/input.csv'
const OUTPUT_PATH = 'data/output.csv'

const validCSV = `topic,status
Math,done
Physics,pending`
...
describe('CSVService', () => {
  ...
  describe('readInputCSV', () => {
    it('should parse a valid CSV file correctly', async () => {
      ...
      expect(rows).toEqual([
        { topic: 'Math', status: 'done' },
        { topic: 'Physics', status: 'pending' }
      ])
    })
  ...
  describe('writeOutputCSV', () => {
    ...
    // Ajout dans csv.service.test.ts
    it('should handle large CSV files correctly', async () => {
      // Créer un CSV volumineux (exactement 1000 lignes)
      const largeCSVContent =
        'subject,topic\n' +
        Array(1000) // Modifier ici de 1001 à 1000
          .fill(0)
